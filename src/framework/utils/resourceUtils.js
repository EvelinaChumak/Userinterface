...
const TEST_DATA_PATH = `${process.cwd()}/src/testData`;
...
module.exports = class ResourceUtils {
  ...
  /**
  * Extract value from object by jsonpath like string or array
  * @param {*} object object extract from
  ...
  */
  static extractValueFromObjectByPath(object, varPath) {
    const assertKeyInObject = (ref, key) => {
      if (!(key in ref)) {
        throw new Error(`Object (${key}) does not exists in path: ${varPath}\n${JSON.stringify(object, null, SPACE)}`);
      }
    };
   ...
   */
  static findFileByPathPart(pathFragments) {
    const tempFragments = [...pathFragments];
    const jsonPath = [];
    while (tempFragments.length) {
      const fragment = tempFragments.pop();
      const filePath = path.join(TEST_DATA_PATH, ...tempFragments, `${fragment}.json`);
  ...
  static resolveVariableBase(variable) {
    ...
    const { filePath, jsonPath } = this.findFileByPathPart(cleanVariable.split('.'));
